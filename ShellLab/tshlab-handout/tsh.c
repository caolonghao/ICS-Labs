...
/*
 * waitpd - used for waiting the foreground to finish
 */
...
eval(char *cmdline) 
{
    ...
    if(tok.outfile!=NULL)
    {
        int fd=open(tok.outfile,O_RDWR|O_CREAT,0);
        if(fd<0)
        {
            sio_puts("Open file error!");
            return;
    ...
    if(tok.builtins)
    {
        ...
        switch(choice)
        {
            case BUILTIN_BG:
            {
                ...
                {
                    target=atoi(s+1);
                    struct job_t* job_now=getjobjid(job_list,target);
                    if(job_now==NULL)
                    {
                        sio_puts("Error: Can't find the job in job list!\n");
                        break;
                    }
                    job_now->state=BG;
                    sio_put("[%d] (%d) ",target,job_now->pid);
                    sio_puts(job_now->cmdline);
                    sio_puts("\n");
                ...
                {
                    target=atoi(s);
                    struct job_t* job_now=getjobpid(job_list,target);
                    if(job_now==NULL)
                    {
                        sio_puts("Error: Can't find the process!\n");
                        break;
                    }
                    job_now->state=BG;
                    sio_put("[%d] (%d) ",job_now->jid,target);
                    sio_puts(job_now->cmdline);
                    sio_puts("\n");
                    kill(job_now->pid,SIGCONT);
                }
            ...
            case BUILTIN_FG:
            {
                ...
                if(s[0]=='%')   //job
                {
                    ...
                    job_now->state=FG;
                /*    sio_put("[%d] (%d) ",job_now->jid,job_now->pid);
                    sio_puts(job_now->cmdline);
                    sio_puts("\n");*/
                    kill(-job_now->pid,SIGCONT);
                    waitfg(job_now->pid);
            ...
            case BUILTIN_NOHUP:
            {
                nohup_target=tok.argv[1];
                nohup=1;
                goto nohup_process;//goto fork the new process with sighup blocked
                break;
            }
        }
    }
    else    //run_program
    {
        nohup_process:;
        pid_t pid;
        sigset_t mask_all,mask_three,prev_one;
        sigfillset(&mask_all);
        ...
        if( (pid = Fork())==0 ) //Child
        {
            ...
            if(nohup)
            {
                ...
                sigprocmask(SIG_SETMASK,&mask,NULL);
                if(execve(nohup_target,tok.argv,environ) < 0)
                {
                    printf("%s: Command not found.\n",tok.argv[0]);
                    exit(0);
                }
            }
            else if(execve(tok.argv[0],tok.argv,environ) < 0)
            {
                printf("%s: Command not found.\n",tok.argv[0]);
                exit(0);
            }
        ...
        //block all signals in case sigint comes before addjob
...
sigchld_handler(int sig) 
{
    ...
    sigfillset(&mask);
//    sio_puts("+2s\n");
    //Problem: after sigint sent to child, it didn't come here
    ...
    while( ( pid = waitpid(-1,&status,WNOHANG | WUNTRACED) ) > 0)
    {
        ...
        else if(WIFSTOPPED(status))
        {
            ...
            struct job_t* job_now=getjobpid(job_list,pid);
            job_now->state=ST;
        }
